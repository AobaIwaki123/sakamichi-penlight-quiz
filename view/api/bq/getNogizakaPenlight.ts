...
import { nogizakaPenlightMock } from './mockData/nogizakaPenlightMock';
...
/**
 * BigQueryから乃木坂46のペンライト色情報を取得する関数
 * USE_MOCK環境変数がtrueの場合はモックデータを返し、falseの場合はBigQueryから取得する
 * 
 * @returns Promise<PenlightColor[]> 乃木坂46ペンライト色情報の配列
 * @throws Error BigQuery接続エラーまたはクエリ実行エラー
 * 
 * @example
 * ```typescript
 * const colors = await getNogizakaPenlight();
 * console.log(`取得した色数: ${colors.length}`);
 * colors.forEach(color => console.log(`${color.name_ja}: ${color.color}`));
 * ```
 */
export async function getNogizakaPenlight(): Promise<PenlightColor[]> {
  const group = 'nogizaka';
  const apiName = 'getNogizakaPenlight';
  ...
  if (environment.useMock) {
    ...
    logApiComplete(apiName, nogizakaPenlightMock.length);
    return nogizakaPenlightMock;
  }
  ...
  try {
    ...
    return handleApiError(apiName, error as Error, nogizakaPenlightMock);
  }
}