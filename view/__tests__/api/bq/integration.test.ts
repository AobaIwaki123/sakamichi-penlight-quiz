...
const mockGetHinatazakaMember = getHinatazakaMember as jest.MockedFunction<typeof getHinatazakaMember>;
const mockGetSakurazakaMember = getSakurazakaMember as jest.MockedFunction<typeof getSakurazakaMember>;
const mockGetHinatazakaPenlight = getHinatazakaPenlight as jest.MockedFunction<typeof getHinatazakaPenlight>;
const mockGetSakurazakaPenlight = getSakurazakaPenlight as jest.MockedFunction<typeof getSakurazakaPenlight>;
...
describe('BigQuery API統合関数', () => {
  ...
  describe('getCompleteDataByGroup', () => {
    ...
    test('API呼び出しエラー時も適切にエラーが伝播される', async () => {
      ...
      await expect(getCompleteDataByGroup('hinatazaka')).rejects.toThrow('API呼び出しエラー');
    });
  });
  ...
  describe('エラーハンドリング', () => {
    test('メンバーAPI呼び出し時のエラーが適切に処理される', async () => {
      ...
      await expect(getMembersByGroup('hinatazaka')).rejects.toThrow('BigQuery接続エラー');
    });
    ...
    test('ペンライトAPI呼び出し時のエラーが適切に処理される', async () => {
      ...
      await expect(getPenlightByGroup('hinatazaka')).rejects.toThrow('クエリ実行エラー');
    });
  });
});
